...
public class ObjectPool : MonoBehaviour
{
    ...
    public GameObject CreateObject(string key, GameObject go)
    {
        ...
        if (cache.ContainsKey(key))
        {
            ...
            {
                if (cache[key][i] != null && !cache[key][i].activeSelf)
                {
                    tempgo = cache[key][i];
                    break;
                }
        ...
        //tempgo = cache.ContainsKey(key) ? cache[key].Find(p => !p.activeSelf) : null; //返回池中未激活的对象，所有都被激活就返回空，赋值给临时对象
        if (tempgo == null)                                                                      //如果临时对象不为空
        {
            tempgo = Instantiate(go); //那就根据传入的预设物，生成一个新物体
